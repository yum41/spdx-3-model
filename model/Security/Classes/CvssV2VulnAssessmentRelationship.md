...
脆弱性に対するCVSSバージョン2.0の評価を提供する。

## Description @ja

CvssV2VulnAssessmentRelationshipは、[A Complete Guide to the Common Vulnerability Scoring System Version 2.0](https://www.first.org/cvss/v2/guide)(CVSSバージョン2.0完全ガイド)に定義される、決定されたスコアと脆弱性のベクトルを記述する。

これは、CVSS calculatorを使用した結果を伝えることを目的としている。

*内容*

- relationshipTypeは`hasAssessmentFor`を設定しなければならない
