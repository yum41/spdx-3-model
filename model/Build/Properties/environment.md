...
ビルドが呼び出されたセッションを記述するプロパティ

## Description @ja

`environment` は、ビルドセッション中に `buildType` に応じて設定される環境変数と値のマップである。

これは [`parameter`](parameter.md) プロパティとは異なり、ビルドが呼び出される前に設定される環境変数を記述するもので、ビルダーに渡される変数ではない。