...
ビルド構成ソースファイルのURIを記述するプロパティ

## Description @ja

ビルドを実行するツールチェーンまたはプラットフォームに対してビルド構成が存在する場合、ビルドの `configSourceUri` は、`buildType` に応じてそのビルド構成の URI となる。

たとえば、GitHub Actionによってトリガーされたビルドは、ビルド構成のYAMLファイルによって定義されます。この場合、`configSourceUri`はそのYAMLファイルのURLとなる。